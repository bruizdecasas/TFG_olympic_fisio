		...
		<div class="contenedor">
			<form class="form" action="/usuario/editar" method="post">
				<fieldset class="full-form">
					<legend>Introduce los nuevos datos ${usuario.nombreUsuario }
						${usuario.apellidosUsuario }</legend>
					<fieldset class="default">
						<input type="hidden" name="idUsuario" id="idUsuario" required
							value="${ usuario.idUsuario }" /> <input type="hidden"
							name="password" id="password" required
							value="${ usuario.password }" />
						<fieldset class="default">
							<label for="emailUsuario">Email</label> <input type="text"
								name="emailUsuario" id="emailUsuario" required
								value="${usuario.emailUsuario }" class="campos" />
						</fieldset>
						<fieldset class="default">
							<label for="nombre_usuario">Nombre</label> <input type="text"
								name="nombreUsuario" id="nombreUsuario" required
								value="${usuario.nombreUsuario }" class="campos" /> <label
								for="apellido_usuario">Apellido</label> <input type="text"
								name="apellidosUsuario" id="apellidosUsuario" required
								value="${usuario.apellidosUsuario }" class="campos" /> <label
								for="fecha_nacimiento">Fecha Nacimiento</label> <input
								type="date" name="fechaNacimiento" id="fechaNacimiento" required
								value="${usuario.fechaNacimiento }" class="campos" />
						</fieldset>
						<fieldset>
							<label for="telefono">Teléfono</label> <input type="text"
								name="telefono" id="telefono" required
								value="${usuario.telefono }" class="campos" /> <label
								for="sociedadMedica">Sociedad Médica </label> (opcional) <input
								type="text" name="sociedadMedica" id="sociedadMedica"
								value="${usuario.sociedadMedica }" class="campos" />
						</fieldset>
						<fieldset>
							<sec:authorize access="hasAuthority('Administrador')">
								...
								<select name="idRol" id="idRol" required class="campos">
									<option value="3" selected class="campos">Seleccionar
										rol</option>
